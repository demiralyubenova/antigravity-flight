...
export default function Create() {
  ...
  // Re-hydrate item objects once wardrobe or suggestions load from persistence
  useEffect(() => {
    if (wardrobeItems.length === 0 || outfitSuggestions.length === 0) return;

    // Check if we have items that need hydration (id present but item object missing)
    const needsHydration = outfitSuggestions.some(o =>
      o.itemIds.length > 0 && (!o.items || o.items.length === 0)
    );

    if (needsHydration) {
      setOutfitSuggestions(prev =>
        prev.map(o => {
          if (o.itemIds.length > 0 && (!o.items || o.items.length === 0)) {
            return {
              ...o,
              items: o.itemIds
  ...
  }, [wardrobeItems, outfitSuggestions]);
